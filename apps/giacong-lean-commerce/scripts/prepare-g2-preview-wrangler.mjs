...
const aud = requiredEnv("G2_PREVIEW_ACCESS_AUD");
...
try {
  ...
  throw new Error(`${sourcePath} must remain JSON-compatible so the G2 preview config can be prepared safely: ${error instanceof Error ? error.message : String(error)}`);
}
...
assert.equal(config.vars.POLICY_AUD, productionAud, "Preparing G2 preview config must not alter Production POLICY_AUD.");
assert.equal(config.vars.ADMIN_HOSTNAME, productionAdminHostname, "Preparing G2 preview config must not alter Production ADMIN_HOSTNAME.");
assert.equal(config.vars.ADMIN_HOSTNAMES, productionAdminHostnames, "Preparing G2 preview config must not alter Production ADMIN_HOSTNAMES.");
assert.equal(config.vars.STAGING_DIRECT_QA_MODE, productionDirectQaMode, "Preparing G2 preview config must not alter Production STAGING_DIRECT_QA_MODE.");
assert.equal(config.env.staging.preview_urls, true, "G2 preview config must explicitly enable preview URLs.");
assert.equal(config.env.staging.workers_dev, false, "G2 preview config must keep the root workers.dev route disabled.");
