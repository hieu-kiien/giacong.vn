...
test("runs focused admin, contact, lead persistence, catalog, news, service, and acceptance tests before lint in the standard check contract", async () => {
  const packageJson = JSON.parse(await readFile(new URL("package.json", root), "utf8"));
  assert.equal(
    packageJson.scripts.check,
    "npm run test:admin && npm run test:contact && npm run test:lead-persistence && npm run test:catalog && npm run test:catalog-purchase-ui && npm run test:news && npm run test:service"
      + " && npm run test:commerce && npm run test:header"
      + " && npm run test:listing && npm run test:detail && npm run test:acceptance"
