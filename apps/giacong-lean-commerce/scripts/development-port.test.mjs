...
test("runs focused admin, contact, lead persistence, catalog, and service tests before lint in the standard check contract", async () => {
  const packageJson = JSON.parse(await readFile(new URL("package.json", root), "utf8"));
  assert.equal(
    packageJson.scripts.check,
    "npm run test:admin && npm run test:contact && npm run test:lead-persistence && npm run test:catalog && npm run test:catalog-purchase-ui && npm run test:service"
