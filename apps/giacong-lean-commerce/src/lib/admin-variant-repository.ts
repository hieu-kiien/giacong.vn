...
interface Statement { bind(...values: unknown[]): Statement; all<T = Record<string, unknown>>(): Promise<{ results: T[] }>; first<T = Record<string, unknown>>(): Promise<T | null>; run(): Promise<D1Result>; }
interface D1Result { success?: boolean; meta?: { changes?: number }; results?: unknown[]; }
interface Database { prepare(sql: string): Statement; batch<T extends Statement>(statements: T[]): Promise<D1Result[]>; }
...
export async function updateVariant(id:number,raw:unknown,actorSubject:string,requestId:string):Promise<AdminVariant>{
  ...
  await db.prepare(`INSERT INTO admin_audit_log(request_id,actorSubject,action,entity_type,entity_key,previous_revision,resulting_revision,payload_sha256) VALUES(?,?, 'update','variant',?,?,?,?)`).bind(requestId,actorSubject,String(id),version,version+1,hash).run();
...
export async function replaceTierPrices(variantId:number,raw:unknown,actorSubject:string,requestId:string):Promise<AdminVariant>{
  ...
  const statements:Statement[]=[
    // Guard the parent first. A stale request must leave tier rows and audit untouched.
    db.prepare("UPDATE product_variants SET revision=revision+1,updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=? AND revision=?").bind(variantId,version),
    db.prepare("DELETE FROM variant_tier_prices WHERE variant_id=? AND EXISTS (SELECT 1 FROM product_variants WHERE id=? AND revision=?)").bind(variantId,variantId,nextVersion),
  ];
  for(const tier of tiers) statements.push(db.prepare("INSERT INTO variant_tier_prices(variant_id,min_quantity,price,currency) SELECT ?,?,?,? WHERE EXISTS (SELECT 1 FROM product_variants WHERE id=? AND revision=?)").bind(variantId,tier.minQuantity,tier.price,"VND",variantId,nextVersion));
  statements.push(db.prepare(`INSERT INTO admin_audit_log(request_id,actor_subject,action,entity_type,entity_key,previous_revision,resulting_revision,payload_sha256) SELECT ?,?,'update','tier_prices',?,?,?,? WHERE EXISTS (SELECT 1 FROM product_variants WHERE id=? AND revision=?)`).bind(requestId,actorSubject,String(variantId),version,nextVersion,hash,variantId,nextVersion));

  const results=await db.batch(statements);
  if((results[0]?.meta?.changes??0)!==1)throw new AdminVariantConflictError("Variant was changed by another operator.");

  const updated=await getVariant(variantId);if(!updated)throw new AdminVariantNotFoundError("Variant not found.");return updated;
}
