...
export async function PATCH(request: Request, context: RouteContext): Promise<Response> {
  ...
  if (!parsed.input || !isRecord(body) || typeof body.expectedRevision !== "number" || !Number.isInteger(body.expectedRevision)) {
    return adminFailure(crypto.randomUUID(), 422, "VALIDATION_ERROR", "Cần thông tin thành viên và expectedRevision hợp lệ.", parsed.fieldErrors);
  }
  try {
    ...
    return adminSuccess(crypto.randomUUID(), { member });
  } catch (error) {
