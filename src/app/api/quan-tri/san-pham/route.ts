...
export async function GET(request: Request) {
  try {
    const url = new URL(request.url); if ([...url.searchParams.keys()].some((key) => !allowed.has(key))) return adminFailure(new Error());
    ...
    if (q) { if (q.length > 100) return adminFailure(new Error()); query.set("q", q.trim()); }
    if (page) { if (!/^[1-9]\d*$/.test(page)) return adminFailure(new Error()); query.set("page", page); }
    if (perPage) { const value = Number(perPage); if (!Number.isInteger(value) || value < 1 || value > 48) return adminFailure(new Error()); query.set("per_page", perPage); }
    if (sort) { if (!sorts.has(sort)) return adminFailure(new Error()); query.set("sort", sort); }
    if (direction) { if (direction !== "asc" && direction !== "desc") return adminFailure(new Error()); query.set("direction", direction); }
