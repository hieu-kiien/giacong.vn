...
export async function getAdmin(): Promise<AdminIdentity | null> { const response = await result("me"); return response.status === 200 ? parseAdminIdentity(response.payload) : null; }
