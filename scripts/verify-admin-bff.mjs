...
try {
  ...
  try { const page = await browser.newPage(); await page.context().addCookies([{ name: "laravel_session", value: "catalog-only", domain: "localhost", path: "/" }, { name: "XSRF-TOKEN", value: "abc%2520token", domain: "localhost", path: "/" }]); const meBefore = requests.filter((request) => request.path.endsWith("/me")).length; await page.goto(`${origin}/quan-tri/san-pham?q=bot&sort=name&page=2`, { waitUntil: "networkidle" }); assert.equal(requests.filter((request) => request.path.endsWith("/me")).length, meBefore + 1, "A protected render must memoize /me within the request."); const previous = page.getByText("Trước", { exact: true }); assert.equal(await previous.evaluate((element) => element.tagName), "SPAN"); assert.equal(await previous.getAttribute("aria-disabled"), "true"); const display = await page.locator("#admin-nav").evaluate((element) => getComputedStyle(element.parentElement).display); assert.equal(display, "grid", "Desktop shell must use a grid parent instead of normal-flow sidebar."); await page.getByRole("button", { name: "Đăng xuất" }).last().click(); const logoutAlert = page.getByText(/Không thể đăng xuất|Không thể kết nối/); await logoutAlert.waitFor(); assert.match(await logoutAlert.textContent() ?? "", /Không thể đăng xuất|Không thể kết nối/); assert.match(page.url(), /\/quan-tri\/san-pham/); await page.close(); const mobile = await browser.newPage({ viewport: { width: 390, height: 844 } }); await mobile.context().addCookies([{ name: "laravel_session", value: "catalog-only", domain: "localhost", path: "/" }]); await mobile.goto(`${origin}/quan-tri/san-pham`, { waitUntil: "networkidle" }); const menu = mobile.getByRole("button", { name: "" }).first(); await menu.press("Enter"); assert.equal(await menu.getAttribute("aria-expanded"), "true"); assert.equal(await mobile.locator("#admin-nav").isVisible(), true); await mobile.keyboard.press("Tab"); assert.equal(await mobile.locator("#admin-nav").evaluate((element) => element.contains(document.activeElement)), true); await mobile.close(); } finally { await browser.close(); }
